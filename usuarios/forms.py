...
class CadastroForms(forms.Form):
  ...
  senha_principal=forms.CharField(
    label='Senha',
    required=True,
    max_length=70,
    widget=forms.PasswordInput(
  ...
  def clean_senha_confirma(self):
    senha_principal = self.cleaned_data.get("senha_principal")
    senha_confirma = self.cleaned_data.get("senha_confirma")
    if senha_principal and senha_confirma:
      if senha_principal != senha_confirma:
